...
function isWeakNormalizedSpace(space: Record<string, unknown>): boolean {
  ...
  return genericTitle || genericCopy || (type === "other" && (genericTitle || genericCopy));
}
...
function buildDeterministicFallbackPlan(input: {
  ...
  if (livingDining) {
    spaces.push(
      buildDeterministicSpace({
        ...
        explanation:
          "结合客餐厅与相邻阳台的关系，优先放大公共区的采光、动线和日常收纳能力，让家庭活动和会客都更舒展。",
        layoutSuggestion:
          "沙发与餐桌沿主要采光面组织，餐边柜和玄关柜尽量一体化延展，减少零碎突出的柜体对通行动线的影响。",
  ...
  if (hasKeyword(combinedText, ["书房", "客房", "多功能", "办公", "临时住", "墨菲"])) {
    spaces.push(
      buildDeterministicSpace({
        ...
        explanation:
          "这个空间优先承担办公阅读与临时留宿两种角色，因此需要在书桌、隐藏床位和整面收纳之间取得平衡，而不是做成单一卧室。",
  ...
  if (hasKeyword(combinedText, ["孩子", "儿童", "学习", "玩耍", "绘本"])) {
    spaces.push(
      buildDeterministicSpace({
        ...
        explanation:
          "儿童房需要从当前的玩耍陪伴场景出发，同时预留后续学习桌、绘本与玩具收纳的位置，避免很快被二次改造。",
  ...
  if (hasKeyword(combinedText, ["阳台", "洗烘", "家政", "公卫", "卫生间", "干湿分离"])) {
    spaces.push(
      buildDeterministicSpace({
        ...
        explanation:
          "阳台和卫浴相关空间承担洗烘、清洁与日常整理任务，重点不是装饰感，而是把设备位、杂物收纳和清爽动线组织清楚。",
        layoutSuggestion:
          "洗衣机与烘干机尽量叠放或并排靠边布置，侧边配置家政高柜；若连着公卫，优先强化干湿分离和镜柜收纳。",
  ...
  return {
    overallStrategy:
      "围绕户型本身的采光、动线和高频生活需求，优先把公共区通透感、关键收纳和复合功能空间梳理清楚，再统一整体风格语汇。",
    styleSummary,
    budgetAssumptions:
...
function normalizeDesignPlan(raw: unknown): unknown {
  ...
  const keySpaces = Array.isArray(source.keySpaces)
    ? source.keySpaces.map((item) => {
        ...
        const designGoal = asString(space.designGoal) ?? designPoints[0] ?? `${title}的功能优化与风格统一`;
        const explanation =
          asString(space.explanation) ??
          `${title}以${designPoints.slice(0, 2).join("，") || "提升使用体验"}为核心，结合当前户型信息做原木风方向设计。`;
        const layoutSuggestion =
          asString(space.layoutSuggestion) ?? designPoints[0] ?? `${title}优先保证通行、采光和主要功能布局。`;
        ...
        const fallbackPaletteAndMaterials = [`${title}以浅木色、耐用饰面和柔和基础色保持整体温暖感。`];
        const fallbackFurnitureAndSoftDecor = [
          `${title}优先选择线条简洁、体量克制的家具，并用基础软装维持舒适度。`
        ];
        const budgetTradeOffs =
          asString(space.budgetTradeOffs) ?? `优先把预算投入在${title}的高频使用面、基础收纳和耐用材质上。`;
...
function ensureStrongNormalizedPlan(raw: unknown, fallbackPlan?: unknown): unknown {
  ...
  if (keySpaces.length === 0 || weakCount >= Math.ceil(keySpaces.length / 2)) {
    if (fallbackPlan) {
      return normalizeDesignPlan(fallbackPlan);
    }
...
export class DeepSeekTextClient {
  ...
  private async chatJson(input: {
    ...
    if (input.logLabel) {
      console.info(`[DeepSeekTextClient] ${input.logLabel} raw response:\n${rawText}`);
    }
  ...
  async nextAgentTurn(input: AgentTurnInput): Promise<AgentTurnOutput> {
    ...
    const currentTurn = Math.min(
      Math.max(
        input.conversation.filter((message) => message.role === "agent").length + 1,
        1
      ),
      12
    );
  ...
  async generateDesignPlan(input: Parameters<AiProvider["generateDesignPlan"]>[0]) {
    ...
    const deterministicFallbackPlan = buildDeterministicFallbackPlan(input);
    ...
    try {
      ...
      if (!finalQuality.isStrong) {
        console.warn(
          `[DeepSeekTextClient] design-plan final output still weak after retries: keySpaces=${finalQuality.keySpacesLength}, weakCount=${finalQuality.weakCount}`
        );
      }

      return designPlanSchema.parse(
        ensureStrongNormalizedPlan(finalQuality.normalized, deterministicFallbackPlan)
      );
    }
  }
}
